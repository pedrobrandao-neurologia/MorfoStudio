...
async function enableProductionMode(textureF16Flag = true, isNvidia = true) {
  // Morfo Studio: permite escolher backend (webgl/cpu) e cai para CPU se WebGL indisponível no worker
  let ok = false
  try { ok = await tf.setBackend(preferredBackend) } catch (e) { ok = false }
  if (!ok && preferredBackend !== 'cpu') {
    callbackUI('WebGL indisponível neste worker; usando CPU (mais lento)', -1)
