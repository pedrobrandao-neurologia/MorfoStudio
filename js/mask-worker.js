...
// ao volume de intensidade e normaliza [p2,p98]→[0,255] dentro dela.

import { maskCleanup, normalizeWithinMask } from './fsl-prep.js'
...
self.onmessage = (e) => {
  ...
  try {
    post({ cmd: 'progress', message: 'Limpando máscara cerebral (limiar, fechamento, componente)…', frac: 0.3 })
    ...
    if (intensity) {
      post({ cmd: 'progress', message: 'Aplicando máscara ao volume…', frac: 0.7 })
